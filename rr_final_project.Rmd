...
__Step 3:__ After informally cataloging the large number of weather event types (using `table(noaa$EVTYPE)`), it became clearly necessary not only to clean up the many errors and disparities that abound in the data but also gather the event type data into categories so that similar events could be examined and assessed collectively. The reasoning here was that similar events often develop under similar conditions and share a profile in terms of damages, for example flooding from heavy rain and flooding from storm surge. To accomplish this, several grouping functions were implemented that rely on regular expressions.
...
kable(perf_storms, caption="Observations Attributable to Super-Storms by Catalogues Event Category")
```
...
To gain some insight into how events are categorized in the dataset, individual storms will be focused on. 
